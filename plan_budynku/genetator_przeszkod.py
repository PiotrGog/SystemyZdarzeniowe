...
def main():
    ...
    for i in range(pietra):
        ...
        plan=separator(read_file(nazwa))

        czysty_plan=copy.deepcopy(plan)
        #print_in_terminal(czysty_plan)

        plan=add_object(plan, ludzie, CZLOWIEK)
        plan=add_object(plan, przeszkody, PRZESZKODA)
        ...
        if i==0:
            ...
            tryb='a'
        save(nazwa_do_zapisu, pietra, tryb, i, ludzie, przeszkody, plan, czysty_plan)
